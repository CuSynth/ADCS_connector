...
def val_from(frame, offset, leng, dtype='i', byteorder="little", signed=False, debug=False):
    """
    ...
    if dtype == 'd' or dtype == 'f':
        byte_arr = bytes(frame[offset + 0:offset + leng])
        retval = struct.unpack(dtype, byte_arr)[0]
    elif dtype == 'i':
